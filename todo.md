...
## Department Separation — Outbound vs Retention (Apr 24 2026)
- [x] Add `department` enum column to users table (outbound | retention, nullable)
- [x] Run pnpm db:push to migrate schema
- [ ] Add `updateUserDepartment` tRPC mutation (admin only) to assign department to agents
- [ ] Add department filtering to getLeaderboard — agents see only their own department
- [ ] Add department filtering to getMyCoachingDashboard — uses user's own department
- [ ] Add department-aware empty state in Opening Hub (Training page) for unassigned users
- [ ] Add department-aware empty state in Workspace for unassigned users
- [ ] Admin UI: add Department dropdown per agent in admin panel (Users/Agents management)
- [ ] Expose `department` in `pitch.allUsers` query for admin panel

## Remove Leaderboard from Navigation
- [x] Remove Leaderboard from TopNav mainNavItems (desktop)
- [x] Remove Leaderboard from TopNav mobileNavItems (mobile bottom bar)
