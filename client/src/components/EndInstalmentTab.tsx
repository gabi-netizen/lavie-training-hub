...
function calcPaidSoFar(sub: MyClientSubscription): number {
  ...
  const cycles = sub.currentBillingCycle ?? sub.billingCycles ?? 0;
  return setup + recurring * cycles;
}

/**
 * Calculate "Outstanding" = totalAmount - paidSoFar (should be 0 for completed installments)
 */
function calcOutstanding(sub: MyClientSubscription): number | null {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-white border border-purple-200 rounded-xl p-4 shadow-sm">
          <div className="text-xs font-semibold text-purple-700 uppercase tracking-wide mb-1">Completed Instalments</div>
          <div className="text-2xl font-bold text-purple-800">{summary.expired}</div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-purple-700 border border-purple-300 rounded-lg hover:bg-purple-50 transition-colors disabled:opacity-50"
        >
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="font-medium text-gray-800">
          Showing {subscriptions.length} of {totalCount} completed instalments
      ...
      {subscriptions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-base font-semibold text-gray-800 mb-1">No completed instalments</h3>
          <p className="text-sm text-gray-600 max-w-sm">
            No expired/completed instalment plans found for your clients.
          </p>
        </div>
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-x-auto">
          ...
          <div
            className="grid items-center gap-1 px-3 py-3 border-b border-gray-200 bg-gray-50 min-w-[1440px]"
            style={{ gridTemplateColumns: "36px 36px 140px 160px 120px 75px 75px 75px 85px 80px 85px 140px" }}
          >
          ...
          {subscriptions.map((sub, idx) => {
            ...
            const outstanding = calcOutstanding(sub);

            return (
              <div key={sub.subscriptionId}>
                <div
                  onClick={() => setExpandedRow(isExpanded ? null : sub.subscriptionId)}
                  className={`grid items-center gap-1 px-3 py-2.5 border-b border-gray-100 cursor-pointer transition-colors hover:bg-gray-50 min-w-[1440px] ${
                    isExpanded ? "bg-purple-50" : ""} ${isSelected(sub.subscriptionId) ? "ring-2 ring-inset ring-blue-400 bg-blue-50" : ""}`}
                  style={{ gridTemplateColumns: "36px 36px 140px 160px 120px 75px 75px 75px 85px 80px 85px 140px" }}
                >
                  {/* Checkbox */}
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-slate-900">
                          {sub.billingCycles != null
                            ? `${sub.currentBillingCycle ?? sub.billingCycles} of ${sub.billingCycles} payments (Complete)`
                      ...
                      <div className="mt-4">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs font-medium text-green-700">
                            {sub.currentBillingCycle ?? sub.billingCycles} / {sub.billingCycles} payments (Complete)
                          </span>
                        </div>
