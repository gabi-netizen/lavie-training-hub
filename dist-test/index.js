...
var users, callAnalyses, aiFeedback, contacts, contactCallNotes, phoneNumbers, emailTemplates, emailLogs, pitchCustomizations, formSubmissions, leadAssignments, callAttempts, gmailIncomingEmails, supportTickets, openingTrials, agentWorkingDays, manualOverridesLog, agentDailyHours, agentTrialsOverride, supportTicketReplies, blockedSenders, blockedSubjects;
...
async function getMyCoachingDashboard(userId, timeRange = "month") {
  ...
  const complianceAvg = (calls) => {
    ...
    return count2 > 0 ? Math.round(total / count2) : null;
  };
  ...
  const CATEGORY_RULES2 = [
    {
      ...
      coachingDetail: (_k, count2, total) => `You asked the question \u2014 great. But the customer told you exactly what she wanted and you moved on. Every answer she gives you is a door. When she says her concern \u2014 that's your cue to bring in the right product. Tie it back, every time. This happened in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /magic\s*wand/i.test(t2),
      category: "Magic Wand Question",
      coachingDetail: (_k, count2, total) => `The Magic Wand question is your most powerful tool. Customers who answer it are far more likely to close. You skipped it in ${count2} of ${total} calls \u2014 make it non-negotiable on every call.`
    },
    {
      ...
      coachingDetail: (_k, count2, total) => `You can't win a sale you don't ask for. You missed the close attempt in ${count2} of ${total} calls. Every call needs a clear, confident close \u2014 even if you think they're not ready. Ask, then stay silent.`
    },
    {
      test: (t2) => /rapport|personal|connect|name|warm/i.test(t2),
      category: "Rapport \u2014 Build the Connection",
      coachingDetail: (_k, count2, total) => `Ask personal questions, use her name, and respond to what she shares. Don't rush to the pitch. A customer who feels heard is 2\xD7 more likely to close. This showed up in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /tone|energy|enthusiasm|excit|flat|monotone|boring|pitch/i.test(t2),
      category: "Tone & Energy \u2014 Bring the Excitement",
      coachingDetail: (_k, count2, total) => `Replace technical language with vivid, sensory words: "feel", "imagine", "wake up with glowing skin". Make her want it before you mention the price. Your pitch fell flat in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /objection|push\s*back|think\s*about|hesitat|overcome|rebut/i.test(t2),
      category: "Objection Handling \u2014 Don't Give Up",
      coachingDetail: (_k, count2, total) => `When a customer says "I need to think about it", don't accept it \u2014 ask which concern it is: the product, or giving card details. Then address that specific concern. You gave up too quickly in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /authenti|script|repeat|robot|natural|filler|absolutely/i.test(t2),
      category: "Authenticity \u2014 You Sound Scripted",
      coachingDetail: (_k, count2, total) => `When you repeat the same word over and over, customers stop trusting you \u2014 it sounds like a script, not a real person. Replace filler words with nothing. Just say "yes", "exactly", or move straight to your next point. You'll sound 10\xD7 more real. Noticed in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /silence|pause|quiet|stop\s*talk/i.test(t2),
      category: "Silence After Close \u2014 Hold the Pause",
      coachingDetail: (_k, count2, total) => `After you ask for the close \u2014 stop talking. The next person who speaks loses. You filled the silence instead of holding it in ${count2} of ${total} calls. Let the pause do the work.`
    },
    {
      test: (t2) => /control|redirect|off.?topic|lead|steer|rambl/i.test(t2),
      category: "Call Control \u2014 Lead the Conversation",
      coachingDetail: (_k, count2, total) => `When a customer goes off-topic, gently redirect: "That's interesting \u2014 let me just finish this one point and we'll come back to that." You're following them instead of leading. This happened in ${count2} of ${total} calls.`
    },
    {
      test: (t2) => /subscri|t\s*&\s*c|terms|compliance|misrepresent/i.test(t2),
      category: "Compliance \u2014 Subscription Handling",
      coachingDetail: (_k, count2, total) => `Never deny or downplay the subscription. The correct response is: "You're in complete control \u2014 cancel anytime with one click or one email." Be proud of the subscription, not defensive. Flagged in ${count2} of ${total} calls.`
    },
    {
      ...
      coachingDetail: (_k, count2, total) => `Customers don't buy ingredients \u2014 they buy results. Paint the picture: "wake up with glowing skin", "feel the difference in 3 days". Use Trustpilot reviews and real results to build trust. Needed in ${count2} of ${total} calls.`
    }
  ];
  function classifyImprovement(key, count2, total) {
    const upper = key.toUpperCase();
    const pct2 = total > 0 ? count2 / total : 0;
    for (const rule of CATEGORY_RULES2) {
      if (rule.test(key)) {
        ...
        return { category: rule.category, detail: rule.coachingDetail(key, count2, total), status };
      }
    }
    const fallbackCategory = key.length > 40 ? key.slice(0, 40).replace(/\s+\S*$/, "...") : key;
    const fallbackDetail = `This came up in ${count2} of ${total} calls. Focus on this area in your next calls \u2014 small changes here will make a big difference to your results.`;
  ...
  const pct = (count2, total) => total > 0 ? Math.round(count2 / total * 100) : 100;
...
var pitchRouter = router({
  ...
  allUsers: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
    return db.select({ id: users.id, name: users.name, email: users.email, role: users.role }).from(users).where(eq7(users.active, true));
  }),
    ...
    for (const row of trialRows) {
      ...
      switch (row.classification) {
        ...
        case "cancelled_after_payment":
          agent.cancelledAfterPayment += count2;
          break;
        case "cancelled_before_payment":
          agent.cancelledBeforePayment += count2;
          break;
        case "dunning":
          agent.dunning += count2;
          break;
        case "future_deal":
          agent.futureDeal += count2;
          break;
      }
    }
...
init_schema();
import { eq as eq13, or as or5, like as like5 } from "drizzle-orm";
...
async function findUserByCloudtalkAgentId2(agentId) {
  ...
  if (!db) return null;
  const agentIdStr = String(agentId);
  const results = await db.select().from(users).where(eq13(users.cloudtalkAgentId, agentIdStr)).limit(1);
  return results[0] ?? null;
}
async function findUserByEmail2(email) {
  const db = await getDb();
  if (!db) return null;
  const results = await db.select().from(users).where(eq13(users.email, email)).limit(1);
  return results[0] ?? null;
}
async function findOrCreateAgentUser2(agentId, agentName, agentEmail) {
  ...
  if (agentEmail) {
    ...
    if (user) {
      await db.update(users).set({ cloudtalkAgentId: agentIdStr }).where(eq13(users.id, user.id));
  ...
  try {
    ...
    const newId = result.insertId;
    const newUsers = await db.select().from(users).where(eq13(users.id, newId)).limit(1);
    ...
    const existing = await db.select().from(users).where(eq13(users.openId, openId)).limit(1);
    return existing[0] ?? null;
  }
}
...
async function isCallAlreadyProcessed2(cloudtalkCallId) {
  const db = await getDb();
  if (!db) return false;
  const results = await db.select({ id: callAnalyses.id }).from(callAnalyses).where(eq13(callAnalyses.cloudtalkCallId, cloudtalkCallId)).limit(1);
  return results.length > 0;
}
...
async function handleCloudTalkWebhook(req, res) {
  try {
    ...
    if (!agent && cloudtalkAgentName) {
      ...
      if (db) {
        ...
        if (nameMatches.length > 0) {
          ...
          const created = await db.select().from(users).where(eq13(users.id, Number(newId))).limit(1);
    ...
    if (!agent) {
      const db = await getDb();
      if (db) {
        const admins = await db.select().from(users).where(eq13(users.role, "admin")).limit(1);
        agent = admins[0] ?? null;
    ...
    processCallAnalysis(analysisId, fileUrl).then(async () => {
      ...
      if (contact) {
        try {
          const db = await getDb();
          if (!db) return;
          const rows = await db.select().from(callAnalyses).where(eq13(callAnalyses.id, analysisId)).limit(1);
...
async function handleGmailWebhook(req, res) {
  try {
    ...
    const existingEmail = await db.select({ id: gmailIncomingEmails.id }).from(gmailIncomingEmails).where(eq14(gmailIncomingEmails.messageId, messageId)).limit(1);
    ...
    try {
      const previousEmails = await db.select({ id: gmailIncomingEmails.id }).from(gmailIncomingEmails).where(eq14(gmailIncomingEmails.fromEmail, fromEmail)).limit(2);
...
init_schema();
import { eq as eq15, sql as sql8, and as and8, inArray as inArray5, desc as desc6 } from "drizzle-orm";
...
async function handlePostmarkInbound(req, res) {
  try {
    ...
    try {
      const blockedRow = await db.select({ id: blockedSenders.id }).from(blockedSenders).where(eq15(blockedSenders.email, fromEmail.toLowerCase())).limit(1);
    ...
    if (isReply) {
      try {
        ...
        if (existingTickets.length > 0) {
          ...
          await db.update(supportTickets).set({ status: "customer_replied" }).where(eq15(supportTickets.id, ticketId));
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    if (db) {
      const updated = await db.update(formSubmissions).set({
        ...
        postcode,
        status: "processed"
      }).where(eq16(formSubmissions.stripePaymentIntentId, pi.id));
      if (!updated) {
        await db.insert(formSubmissions).values({
          email,
          cardholderName,
